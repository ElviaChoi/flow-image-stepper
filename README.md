...
7. **Next scene**으로 장면 이미지를 하나씩 생성합니다.
8. Flow에서는 이미지가 생성됐지만 확장 저장이 꼬였을 때는 **Recover scene**을 사용합니다.
9. 장면을 다시 생성해야 하면 **Back one scene**을 사용합니다.
10. 생성된 장면 이미지를 내려받을 때 **Download scene images**를 누릅니다.
...
- `characterRefs`: 캐릭터 ID별 캐릭터 시트 이미지 연결 정보
- `sceneOutputs`: 장면별 생성 이미지 URL과 파일명
...
```text
Scene output status:
- 001 [done] saved 2: 001_Ch02_Main_CS-02_01, 001_Ch02_Main_CS-02_02
- 002 [next] saved 0: -
- 003 [todo] saved 0: -
```
